...
export default function CartPage() {
  ...
  const handleWhatsAppCheckout = () => {
    ...
    // 2. CONFIGURÁ TU NÚMERO DE WHATSAPP AQUÍ
    // Formato: Código de país (54) + 9 + Código de área (ej: 351) + Número
    // Ejemplo: Si tu número es 351 1234567, ponés "5493511234567" sin espacios ni el "+"
    const phoneNumber = "5493518089416"; 

    // 3. Armamos el mensaje automático
    let message = `¡Hola Polirubro Online! Mi nombre es *${customerName.trim()}* y quiero confirmar la siguiente compra:\n\n`;
    ...
    message += `\n*Total a pagar: $${total.toLocaleString('es-AR')}*\n\n¿Me confirman para realizar el pago?`;

    // 4. Codificamos el texto para que la URL sea válida y abrimos WhatsApp
      ...
      <div className="bg-gray-800 rounded-3xl shadow-xl border border-gray-700 overflow-hidden">
        ...
        <div className="bg-gray-900 p-6 sm:p-10 border-t border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-8">
          ...
          <button 
            ...
            className="w-full sm:w-auto bg-[#25D366] hover:bg-[#1ebe57] text-white font-black text-xl py-5 px-8 rounded-full shadow-lg transition-all transform hover:-translate-y-1 hover:shadow-2xl flex items-center justify-center gap-3"
          >
            {/* Ícono de WhatsApp */}
