...
${JSON.stringify(productsToClassify)}`;
  ...
  // FUNCIÓN GUARDAR: AUTO-CLASIFICACIÓN FRONTEND CON GEMINI
  const saveProduct = async (e) => {
    ...
    try {
      ...
      // Si la categoría está vacía, le preguntamos a Gemini directamente desde acá
      if (!finalCategory || finalCategory.trim() === '') {
        setIsAutoClassifying(true);
        const apiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY;

        if (!apiKey) {
          alert("⚠️ Faltó configurar la llave NEXT_PUBLIC_GEMINI_API_KEY en Vercel. Guardando como 'Sin Clasificar'.");
          finalCategory = "Sin Clasificar";
        } else {
          try {
            const prompt = `
              Eres un experto en e-commerce. Asigna UNA categoría corta y precisa a este producto.
              Sugerencias: "Tecnología y Gaming", "Bazar y Parrilla", "Deportes y Tiempo Libre", "Librería y Estudio", "Accesorios y Telefonía", "Juguetería", "Niños", "Indumentaria", "Ferretería".
              Si no encaja, inventa una categoría corta.
              RESPONDE ÚNICAMENTE CON EL NOMBRE DE LA CATEGORÍA. Sin comillas ni explicaciones.
              
              Producto: ${productForm.name}
              Descripción: ${productForm.description || 'Sin descripción'}
            `;
            ...
            if (res.ok) {
              ...
              const textCat = data.candidates[0].content.parts[0].text;
              finalCategory = textCat.trim().replace(/["']/g, '');
            } else {
              finalCategory = "Sin Clasificar";
              console.error("Fallo la respuesta de Gemini");
            }
          } catch (iaError) {
      ...
      // Procedemos a guardar en Supabase con la categoría final (manual o de IA)
      const productData = {
