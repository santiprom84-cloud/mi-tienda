...
export async function POST(req) {
  try {
    ...
    const prompt = `
      ...
      REGLA: Si no encajan, PUEDES CREAR NUEVAS categorías cortas (Ej: Juguetería, Indumentaria, Niños). 
      Devuelve SOLO un JSON válido: [{"id": "id", "category": "Categoria"}]
      Productos:
    ...
    if (!response.ok) {
      ...
      throw new Error("La API de Gemini rechazó la conexión.");
    }
    ...
    if (!jsonMatch) {
      throw new Error("La IA no devolvió un JSON.");
    }
